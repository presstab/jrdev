"""
TypeScript language parsing utilities for function detection and manipulation.
"""
import re

def parse_typescript_signature(signature: str):
    """
...
def is_control_structure(name):
    """Check if the identified name is a control structure and not a method."""
    return name in CONTROL_STRUCTURES

def find_function_end(lines, start_idx, brace_level):
    """
...
def process_class_method(lines, line_idx, current_class, functions):
    """
    ...
    if is_control_structure(method_name):
        return line_idx, False, "", 0
    ...
    method_end_idx = find_function_end(lines, line_idx + 1, brace_level)
...
def process_standalone_function(lines, line_idx, functions):
    """
    ...
    function_end_idx = find_function_end(lines, line_idx + 1, brace_level)
...
def parse_typescript_functions(filepath):
    """
    ...
    while line_idx < total_lines:
        ...
        if current_class:
            new_idx, in_method, method_name, method_start = process_class_method(
        ...
        if not current_class:
            new_idx, in_method, method_name, method_start = process_standalone_function(
