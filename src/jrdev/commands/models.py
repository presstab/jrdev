...
from jrdev.models import get_available_models, VCU_Value
...
async def handle_models(terminal: Any, args: List[str]) -> None:
    ...
    models_list = await get_available_models()

    # Sort models first by provider, then by name alphabetically
    models = cast(List[ModelInfo], models_list)
    sorted_models = sorted(
        models,
        key=lambda model: (model["provider"], model["name"])
    )
    
    # Use curses-based interactive selector with error handling
    try:
...
def interactive_model_selector(stdscr, terminal, models):
    ...
    try:
        ...
        while True:
            ...
            stdscr.addstr(0, 0, "Select a model (↑/↓ to navigate, Enter to select, q to cancel)", 
                        curses.A_BOLD)
            ...
            for i in range(min(max_rows, len(models) - offset)):
                ...
                # Display the row
                try:
                    model_display = f"{prefix}{model_name:20} | {provider:10} | {think_status:10} | {input_cost_str:12} | {output_cost_str:12} | {context_str:10}"
